...
a, b, c = map(int, input().split()) # 출발 엣지
start = 1 - c

for _ in range(M):
    a, b, c = map(int, input().split()) # a와 b 건물애 연결된 도로가 있다는 뜻, c는 0(오르막길), 1(내리막길)
    edges.append((c, a, b))
...
edges.sort(reverse=True) # 최선의 경로 - 내리막길로 많이 간
min_ = mst()

parent = [i for i in range(N+1)]
edges.sort() # 최악의 경로 - 오르막길로 많이 간
max_ = mst()
